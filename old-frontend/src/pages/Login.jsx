  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/v1/user/login", {
        ...
        credentials: "include", // ← CRITICAL: receives httpOnly cookie
        body: JSON.stringify(credentials),
      });

      const data = await response.json();
      console.log("Login response:", data); // check field names in DevTools

      if (response.ok) {
        // ✅ Tell AuthContext user is logged in — Nav updates instantly
        login({
          name:  data.user?.name  || data.name  || "User",
          email: data.user?.email || data.email || credentials.email,
          role:  "customer", // ← hardcode for customer login page
        });
        navigate("/dashboard");
      } else {
